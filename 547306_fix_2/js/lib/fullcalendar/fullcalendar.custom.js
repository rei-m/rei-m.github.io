...
var Grid = fc.Grid = RowRenderer.extend({
	...
	getDayClasses: function(date) {
		...
		// coconala カレンダー内の選択中の日付を取得
		var selectedDate = view.calendar.selectedDate;
		...
		// coconala 月表示の場合のみ、選択中の日付をハイライト表示
...
Grid.mixin({
	...
	getSegClasses: function(seg, isDraggable, isResizable) {
		...
		// coconala カレンダーの現在日付とイベントの基準日付を取得する
		// イベントは終了時間が設定されている場合は終了時間を、されていない場合は開始時間を基準とする
		var now = this.view.calendar.getNow();
		var today = moment(now.year() +
											'-' + ('0' + (now.month() + 1)).substr(-2) +
											'-' + ('0' + now.date()).substr(-2));
		var base = (event.end === null) ? event.start : event.end;
		base = moment(base.year() +
											'-' + ('0' + (base.month() + 1)).substr(-2) +
											'-' + ('0' + base.date()).substr(-2));
		var classes = [
			'fc-event',
		...
		// coconala イベント情報にオプションクラスが設定されている場合は追加する
		if (event.optionalClass !== void 0) {
			classes.push(event.optionalClass);
		}

		// coconala 当日より前にイベントが終了している場合はend_eventを追加する
		if (0 < today.diff(base, 'days')) {
			classes.push('end_event');
		}
...
var DayGrid = Grid.extend({
	...
	renderDates: function(isRigid) {
		...
		// coconala 日表示のヘッダ部をクリックしたら選択中の日付を更新してViewを更新する
		this.el.find('.fc-header-day').on('click', function(e) {
			view.calendar.selectedDate = moment($(this).data('date'));
			view.calendar.gotoDate($(this).data('date'));
		});
	},
	...
	dayRowHtml: function(row, isRigid) {
		...
		// coconala 日表示のカレンダーの場合はヘッダに選択中の日付の週を標示する
		if (this.view.type === 'agendaDay') {
			var $customHeader = $('<tr>');
			$customHeader.append($('<td>').addClass('fc-axis fc-widget-content'));

			var selectedDate = this.view.calendar.selectedDate;
			var year = selectedDate.year();
			var month = '0' + (selectedDate.month() + 1);
			var day = '0' + selectedDate.date();
			var dow = selectedDate.day();
			var weekStart = moment(year + '-' + month.substr(-2) + '-' + day.substr(-2));
			weekStart.add(dow * -1, 'd');
			for(var i=0; i < 7; i++) {
				var $tdDay = $('<td>').addClass('fc-day fc-widget-content fc-header-day');
				var $spanDay = $('<span>').text(weekStart.date());
				if (i === dow) {
					$tdDay.addClass('fc-header-day-selected');
				}
				$customHeader.append($tdDay.append($spanDay));
				$tdDay.attr('data-date', weekStart.format('YYYY-MM-DD'));
				weekStart.add(1, 'd');
			}
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (seg.isStart) {
			// coconala 時間帯表示のフォーマットを変更
			timeText = this.getEventTimeText(event, "h:mm");
			if (timeText) {
				// coconala spanをdivに変更
				timeHtml = '<div class="fc-time">' + htmlEscape(timeText) + '</div>';
			}
		}
...
function Calendar_constructor(element, overrides) {
	...
	t.option = option;
	t.trigger = trigger;
	// coconala 今日日付を選択中の日付のデフォルト値にセット
	...
	// coconala 選択中の日付のSetter
	t.setSelectDate = function(d) {
		t.selectedDate = d;
	}
	...
	if (options.defaultDate != null) {
		...
		// coconala defaultDateが設定されている場合は選択中の日付をdefaultDateで更新
		t.selectedDate = date;
	}
	else {
		date = t.getNow();
	...
	function prev() {
		...
		// coconala 日単位の表示の場合は選択中の日付を更新
		if(this.view.type === 'agendaDay') {
			this.view.calendar.setSelectDate(date);
		}
		renderView();
	}
		...
		return '' +
			'<table>' +
				...
				'<tbody class="fc-body">' +
					'<tr>' +
						'<td class="' + this.widgetContentClass + '">' +
						// coconala fc-day-custom-headerクラスを追加
							(this.dayGrid ?
								'<div class="fc-day-grid fc-day-custom-header"/>' +
